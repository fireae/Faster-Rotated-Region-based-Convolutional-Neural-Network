...
from modules.intersection_cpu import intersection_over_union_cpu
...
def non_maximum_suppression(bbox, thresh, score=None,
    ...
    # xp = np
    return _non_maximum_suppression(bbox, thresh, score, limit, xp)
    # if xp == np:
...
def _non_maximum_suppression(bbox, thresh, score=None, limit=None, xp = np):
    if len(bbox) == 0:
    ...
    for i, b in enumerate(bbox):
        # print i
        area = intersection_over_union_cpu(b,bbox[selec,:])
    ...
    print "There are %d ROIs before performing nms" % bbox.shape[0]
    print "There are %d ROIs after performing nms" % np.count_nonzero(selec)
    if score is not None:
        selec = order[selec]
    # raw_input("Press Enter to continue...")
    return selec.astype(xp.int32)
