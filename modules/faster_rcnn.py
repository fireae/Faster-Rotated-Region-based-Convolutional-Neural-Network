...
from modules.non_maximum_suppression import non_maximum_suppression
...
class FasterRCNN(chainer.Chain):
    ...
    def __init__(
            ...
            loc_normalize_std=(0.1, 0.1, 0.2, 0.2, 1),
    ):
    ...
    def _suppress(self, raw_cls_bbox, raw_prob,W,H):
        bbox = list()
        label = list()
        score = list()
        # skip cls_id = 0 because it is the background class
        for l in range(1, self.n_class):
            cls_bbox_l = raw_cls_bbox.reshape(-1, self.n_class, 5)[:, l, :]
            prob_l = raw_prob[:, l]
    ...
    def predict(self, imgs):
        ...
        for img, scale in zip(prepared_imgs, scales):
            ...
            roi_cls_loc = roi_cls_loc.reshape(-1, self.n_class, 5)


            roi = self.xp.broadcast_to(roi[:, None], roi_cls_loc.shape)
            cls_bbox = loc2bbox(roi.reshape(-1, 5), roi_cls_loc.reshape(-1, 5))
            cls_bbox = cls_bbox.reshape(-1, self.n_class * 5)
            # clip bounding box
            ...
            bbox, label, score = self._suppress(raw_cls_bbox, raw_prob,W,H)
            bboxes.append(bbox)
            labels.append(label)
            scores.append(score)
