...
from modules.generate_anchor_base import generate_anchor_base
from modules.proposal_creator import ProposalCreator
...
class RegionProposalNetwork(chainer.Chain):
    ...
    def __init__(
            ...
            anchor_scales=[8, 16, 32], angles = [0,math.pi/6,math.pi/3],feat_stride=16,
            initialW=None,
            proposal_creator_params={},
    ):
        self.anchor_base = generate_anchor_base(
            anchor_scales=anchor_scales, ratios=ratios, angles=angles)
        ...
        super(RegionProposalNetwork, self).__init__(
            ...
            loc=L.Convolution2D(
                mid_channels, n_anchor * 5, 1, 1, 0, initialW=initialW)
        )
    ...
    def __call__(self, x, img_size, scale=1., test=True):
        ...
        * :math:`A` is number of anchors assigned to each pixel.
        ...
        rpn_locs = self.loc(h)
        rpn_locs = rpn_locs.transpose((0, 2, 3, 1)).reshape(n, -1, 5)
...
def _enumerate_shifted_anchor(anchor_base, feat_stride, width, height):
    ...
    shift = xp.stack((shift_x.ravel(), shift_y.ravel(),
                      xp.zeros(width*height), xp.zeros(width*height),xp.zeros(width*height)), axis=1)

    A = anchor_base.shape[0]
    K = shift.shape[0]
    anchor = anchor_base.reshape((1, A, 5)) + \
        shift.reshape((1, K, 5)).transpose((1, 0, 2))
    anchor = anchor.reshape((K * A, 5)).astype(np.float32)
    return anchor

if __name__ == '__main__':
    anchor_base = np.zeros((18, 5),
                           dtype=np.float32)
    result = _enumerate_shifted_anchor(
        anchor_base, 2, 8, 8)
    print result
    print result.shape